...
%DIF DEL EVL3-25-0079-R1-main.tex   Thu Mar  5 15:50:50 2026
%DIF ADD EVL3-25-0079-R2-main.tex   Thu Mar  5 16:29:37 2026
...
\title{\DIFdelbegin \DIFdel{Phylogram instead of chronogram when assessing the neutral evolution of a trait}\DIFdelend \DIFaddbegin \DIFadd{Phylograms better fit neutrally evolving traits than chronograms}\DIFaddend }
...
\DIFdelbegin \DIFdel{Indeed}\DIFdelend \DIFaddbegin \DIFadd{In other words}\DIFaddend , Brownian evolution can also result from non-neutral evolutionary processes, such as \DIFaddbegin \DIFadd{stabilizing selection on an optimum that moves as a BM in time, or such as }\DIFaddend fluctuating natural selection.
...
To disentangle neutral evolution from \DIFaddbegin \DIFadd{different regimes of }\DIFaddend selection, another approach is to contrast the observed rate of evolution to the neutral expectation~\citep{lande_genetic_1980}.
...
When generalized to many species at the phylogenetic scale, contrasting between and within-species variations allows us to disentangle neutral evolution from selection \DIFaddbegin \DIFadd{tracking a moving optimum}\DIFaddend .
...
Instead, to disentangle neutral evolution from selection \DIFaddbegin \DIFadd{tracking a moving optimum }\DIFaddend in the context of phylogenetic comparative methods, we hereby focus on the underlying tree and the unit of its branch lengths (Fig.~\ref{fig:methods}A).
...
\begin{figure*}[!htb]
    ...
    \caption{
        ...
        Comparing the fit of a BM process on both trees can help disentangle \DIFdelbeginFL \DIFdelFL{selection from }\DIFdelendFL neutral evolution \DIFaddbeginFL \DIFaddFL{from selection tracking a moving optimum}\DIFaddendFL .
    }
    \label{fig:methods}
\end{figure*}
%TC:endignore
...
Using genomic information, we seek to provide additional approaches to model trait evolution, particularly to disentangle \DIFdelbegin \DIFdel{selection and neutral evolution }\DIFdelend \DIFaddbegin \DIFadd{neutral evolution from selection tracking a moving optimum}\DIFaddend .
...
From a \DIFdelbegin \DIFdel{frequentist }\DIFdelend \DIFaddbegin \DIFadd{maximum likelihood }\DIFaddend perspective, the support of a phylogram over a chronogram can also be assessed by \DIFaddbegin \DIFadd{model selection, by }\DIFaddend comparing the maximum likelihood of the data given each tree, and computing the Akaike Information Criterion (AIC) weights of each model (see Supplementary Material section~S5).
...
Therefore, a good fit of BM alone cannot distinguish between neutral evolution and \DIFdelbegin \DIFdel{selection}\DIFdelend \DIFaddbegin \DIFadd{stabilizing selection on an optimum that moves as a BM}\DIFaddend , and the comparison between phylogram and chronogram provides a crucial additional diagnostic.

Distinguishing these scenarios is not trivial, but we here showed that the underlying backbone tree to model trait evolution can help disentangle neutral evolution from selection \DIFaddbegin \DIFadd{tracking a moving optimum}\DIFaddend .
...
\DIFdelbegin \DIFdel{Unfortunately, this result suggests that for a single trait, unless the signal is strong enough, many speciesare required to obtain a statistically significant conclusion}\DIFdelend \DIFaddbegin \DIFadd{This undecided result could reflect insufficient statistical power due to the small number of species, but other explanations are also possible: the true evolutionary process may not conform well to BM on either timescale, or it may involve a combination of selective and neutral processes that does not clearly favour one tree type over the other}\DIFaddend .
...
These examples show that while phylograms are useful to model neutral evolution, \DIFdelbegin \DIFdel{they are not a silver bullet to disentangle the effect of drift and selection}\DIFdelend \DIFaddbegin \DIFadd{caution is still warranted}\DIFaddend .
...
Notably, we did not simulate a static OU process (with a constant optimum), which would presumably favour an OU model; such simulations are outside the scope of this study since our focus is on distinguishing neutral evolution from selection \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{tracking }\DIFaddend a moving optimum.
...
Gene expression levels (both mRNA and protein levels) represent one class of traits where distinguishing neutral evolution from \DIFaddbegin \DIFadd{different regimes of }\DIFaddend selection is particularly important.
...
We propose the phylogram-chronogram comparison as a new tool in the comparative methods toolbox, particularly useful when \DIFdelbegin \DIFdel{seeking deviation from neutral evolution}\DIFdelend \DIFaddbegin \DIFadd{assessing whether a trait is evolving neutrally or under selection tracking a moving optimum}\DIFaddend .
...
For empiricists more familiar with \DIFdelbegin \DIFdel{frequentist }\DIFdelend \DIFaddbegin \DIFadd{maximum likelihood }\DIFaddend approaches, we also provide \DIFdelbegin \DIFdel{a maximum likelihood }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend implementation that yields comparable results through AIC weights (see \DIFdelbegin \DIFdel{Supplementary Material section~S5}\DIFdelend \DIFaddbegin \DIFadd{supplementary material section~S5}\DIFaddend ).
