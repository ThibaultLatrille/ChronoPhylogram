...
By observing the changes in trait along a lineage, it is possible to speculate on the regime of selection acting on it.
But even without selection, a trait might still changes due to random genetic drift.
...
Alleviating statistical artifact and adding biological complexity and realism, the OU process can be relaxed to allow for multiple optima along the phylogenetic tree, which is interpreted as a change in the fitness peak~\citep{ingram_surface_2013, uyeda_novel_2014, khabbazian_fast_2016, mitov_fast_2020, grabowski_cautionary_2023}.
...
This modelling raises an issue: the BM can not be interpreted as the null model of neutral trait evolution since it also models the alternative scenario of a continuously moving optimum.
...
One blind spot of this modelling framework is the underlying tree and the unit of its branch lengths (Fig.~\ref{fig:methods}A).
...
Instead, on a phylogram, the branch lengths can represent another quantity than time, which can be used as the backbone to model trait evolution.
For example, it is known that using phylograms in unit of morphological distance can be used to improve ancestral trait reconstruction for a discrete character~\citep{wilson_chronogram_2022}.
...
Such a phylogram in unit of nucleotide divergence, if obtained from neutrally evolving genomic loci, would absorb the effect of changing generation time and mutation rates, and thus would be more appropriate to model a trait evolving neutrally~\citep{latrille_detecting_2024}.
Conversely, for a trait evolving under selection at the species level, using a chronogram would be more accurate to model mean trait changes since shifts in the optimum fitness peak is extrinsic to the species, and thus should be better correlated to time rather than the number of substitutions that occurred along a branch~\citep{hansen_comparative_2008}.
...
In this study, we test if using a phylogram (in unit of nucleotide divergence for neutral loci) instead of a chronogram can allow to assess the regime of evolution of a trait (Fig.~\ref{fig:methods}B), by evaluating by the fit of a BM on both trees.
More precisely, we seek to test if change of a neutral trait is better predicted a phylogram.
Conversely, we also seek to test if changes of a trait under a moving optimum is better predicted by a chronogram.
As such, by using genomic information we seek to provide additional approaches to model trait evolution, particularly to disentangle selection and neutral evolution.
...
Forth, based in the single rate BM, we derived a model to estimate the support of a phylogram over a chronogram that we apply to the simulated and empirical datasets.
...
Alternatively, we performed simulations with fluctuating mutation rate, generation times and $\Ne$, were we used a BM to model the long-term changes along the phylogenetic tree, and we overlaid short-term changes in $\Ne$ (see supplementary material section~\ref{sec:simulator}).
...
We expect for trait under neutral evolution more accurate trait reconstruction on a phylogram, and conversely for trait under selection (moving optimum) more accurate trait reconstruction on a chronogram.
...
Such model was implemented in \textit{RevBayes} and contains a switch variable, denoted $\PhyloSupport$ that allows the model to switch the branch lengths in units of time to units of substitutions.
...
The posterior mean of $\PhyloSupport$ (burn-in of 1000 gen., running of 10000 gen., 2 chains) is the probability the phylogram is favoured over the chronogram.
We expect $\PhyloSupport = 1$ for trait under neutral evolution, and $\PhyloSupport = 0$ for trait under selection (moving optimum).

\subsection*{Alternatives to single rate BM}
Additionally to a single rate BM, we fitted a BM with multiple rate parameters~\citep[auteur]{eastman_novel_2011}, using either a phylogram or a chronogram as the underlying tree to the simulated dataset.
...
\begin{figure*}[!htb]
    ...
    \caption{
        ...
        Panel~B: Neutral evolution is modelled as a constant fitness regardless of phenotypic value.
        Selection is modelled by stabilizing selection around a optimum value, which is changing along the phylogenetic tree.
        Panel~C: Example simulation of a trait evolving along a phylogenetic tree.
        Panels D \& E: Violinplot of posterior probability of the phylogram being favoured over the chronogram ($\PhyloSupport$) across 100 replicate simulations for the different simulated regime of evolution.
        Horizontal lines inside violins are the estimated $\PhyloSupport$ of each replicate simulation.
        Regime of evolution: neutral regime (yellow), under a moving optimum (blue), and under multiple optima (red).
...
We simulated traits evolving on a phylogenetic trees under different regimes of selection: 1) neutrally evolving, 2) under a moving optimum and 3) under multiple optimum (see Methods, Fig.~\ref{fig:results-brownian}A-C).
We tested for the support of phylogram over a chronogram under the assumption that changes follow a Brownian motion (BM) process, we developed as statistic denoted as $\PhyloSupport$, which range from 0 (full support for the chronogram) to 1.0 (full support for the phylogram).
For simulations with a constant generation time, the support is similar for both a phylogram and a chronogram regardless of the regime of evolution (Fig.~\ref{fig:results-brownian}D), which is expected since the branch length are equivalent in both trees.
...
In this case, for a neutral trait, the BM was better fitted on a phylogram than on a chronogram (Fig.~\ref{fig:results-brownian}E), with an average support for phylogram of $\MeanPhyloSupport = 0.94$ across $100$ replicate simulations, and $80 / 100$ replicates above the $0.95$ threshold.
Conversely, for a trait under selection, the fit of a BM on a chronogram was better than on a phylogram with an average support for phylogram of $\MeanPhyloSupport = 0.17$ for multiple optima ($69 / 100$ replicates below the $0.05$ threshold) and $\MeanPhyloSupport = 0.14$ for a moving optimum ($67 / 100$ below the $0.05$ threshold).
...
Beside support for the underlying tree, we also assessed the accuracy of ancestral trait reconstruction on a phylogram versus a chronogram.
...
Moreover, we tested alternative to the BM process with Ornstein-Uhlenbeck (OU) models: with a single optimum and with multiple optima (see Methods).
...
Altogether, fitting an OU process on a phylogram results in biases, for a trait under selection but even for a trait under neutral evolution.
...
The null model of neutral trait evolution, genetic drift, is though as a Brownian motion (BM) running on the tree~\citep{lynch_phenotypic_1986, felsenstein_phylogenies_1988a}, and deviations from this model are interpreted as selection acting on the trait~\citep{butler_phylogenetic_2004}.
As a result, traits on which the BM is favoured over alternative models are sometimes interpreted as a trait evolving neutrally~\citep{khaitovich_evolution_2006, catalan_drift_2019}.
...
Such identifiability is problematic, but we here argue that the underlying backbone tree to model trait evolution can help to disentangle neutral evolution from selection.
The standard in phylogenetic comparative methods is to use a chronogram, where the branch lengths are measured in time~\citep{felsenstein_phylogenies_1985, harmon_phylogenetic_2018}, while phylograms have been generally overlooked, where the branch lengths are measured in number of substitutions instead.
Such overlook is rational and justified, since for simulated trait under selection we assessed that the BM has better fit on a chronogram than on a phylogram.
However, we here argue that to model a trait evolving neutrally, the backbone tree should not be a chronogram but instead a phylogram.
Such phylogram would better represent the number of generations that occurred, and in turn should explain best the differences in traits between species~\citep{latrille_detecting_2024}
...
Additionally, for the neutral trait, both ancestral state reconstruction and estimation for the number of rate changes for a multi-rate BM was also more accurate on a phylogram.
Altogether, the combined use of a phylogram and a chronogram can help support the hypothesis of neutral evolution, or instead rule it out.
...
Applied to different empirical dataset of body and brain masses in mammals, we showed that the chronogram was favoured over the phylogram, ruling out the neutral model of evolution for these trait~\citep{latrille_detecting_2024}.
...
Moreover, if population structure impacts the probability of fixation for neutral mutations (that would also impact a neutral trait), the phylogram would absorb automatically these changes.
This argument is not only verbal since we tested for it in our simulation by modelling changes in both $\Ne$ and generation time, and the phylogram was favoured over the chronogram for a trait evolving neutrally.
...
Finally, because chronogram and usually derived from chronogram by calibrating the tree with molecular clocks, using phylogram has the additional advantage of removing model assumptions required to calibrate node ages~\citep{litsios_effects_2012}.
...
In our simulations, we found that generally using a phylogram tend to favour the OU process over the Brownian motion (BM) model.
This suggests that trees in unit of nucleotide divergence may not always provide a reliable framework for modelling trait evolution, especially when selection is involved.
As such, we recommend continuing to use chronograms for modelling selection acting on a trait, as they generally provide more accurate and reliable results.
...
Such saturation might be one of the reason for a bias in favour of the Ornstein-Uhlenbeck (OU) process over the Brownian motion (BM) model that we observed in our simulations.
...
Second, while the tested alternative models of evolution (multi-rates BM, OU, multi-optimum OU) are standard, they do not encompass the full breadth of available methods, which could be expanded~\citep{pennell_geiger_2014, hohna_revbayes_2016}.
Finally, the dataset of body and brain masses in mammals may not be representative of all traits or species and even though our analysis showcase the utility of phylograms, more empirical studies are needed to replicate the findings.

% Gene expression level and empirical studies.
One striking example of traits that could (and should) be empirically studied with a phylogram is gene expression level, both for protein concentration and mRNA expression.
...
And typically, dataset on which the BM is favoured over other models is interpreted as mRNA expression level evolving neutrally~\citep{khaitovich_evolution_2006, catalan_drift_2019, dimayacyac_evaluating_2023}.
...
% Conclusion
Altogether, our study support the use a chronogram when testing for selection acting on a trait.
...
We provide a Bayesian model implemented in \textit{RevBayes} to test this hypothesis, which indicate whether a BM support better a phylogram or a chronogram ($0 \leq \PhyloSupport \leq 1$), given data at the tip of the tree and both trees with the same topology.
Only then can we claim that a trait is evolving under drift if the phylogram has more support than chronogram ($\PhyloSupport$ close to $1$), but caution is still warranted.
