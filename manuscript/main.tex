...
However, if the trait is not under natural selection and instead is changing due to the accumulation of neutral substitutions (with the substitution rate equal to the mutation rate under neutrality), the genetic differences between species instead of time should best explain the differences in traits.
...
Additionally, we can also derive the expected changes in mean trait value along a lineage given the trait variance at the population scale~\citep{turelli_heritable_1984, turelli_heritable_1988, lynch_genetic_1990, felsenstein_phylogenies_1988a}.
When generalized to many species at the phylogenetic scale, contrasting between and within-species variations allows us to disentangle neutral evolution from selection.
Specifically, \citet{latrille_detecting_2024} showed that the ratio of between-species to within-species trait variation should scale linearly with nucleotide divergence for a neutral trait, providing a test for neutrality that requires both types of variation data.
...
\begin{figure*}[!htb]
    ...
    \caption{
        ...
        Comparing the fit of a BM process on both trees can help disentangle selection from neutral evolution.
    }
    \label{fig:methods}
\end{figure*}
%TC:endignore
...
As a result, variations of generation time between species means that modelling neutral evolution as BM on a chronogram might induce biases~\citep{litsios_effects_2012}.
...
In this study, we test this hypothesis, namely if using a phylogram (in units of nucleotide divergence for neutral loci) instead of a chronogram (in units proportional to time) can allow to assess the regime of evolution of a trait, by evaluating the fit of BM on both trees.
...
To assess which tree type (phylogram vs. chronogram) better supports neutral evolution, we evaluate both the statistical fit of the BM and the accuracy of ancestral trait reconstruction under each tree.
...
The chronogram is the generating ultrametric time tree used in the simulation. For the empirical datasets, the chronogram is derived from the phylogram by fitting a relaxed molecular clock model (correlated rate model with penalized likelihood, default value) to the phylogram with the \textit{R} package \textit{ape}~\citep{paradis_ape_2004}.
...
\begin{figure*}[!htb]
    ...
    \caption{
        ...
        The posterior mean estimate of $\PhyloSupport$ indicates whether BM supports better the phylogram over the chronogram ($0 \leq \PhyloSupport \leq 1$).
...
For a neutral trait, ancestral trait reconstruction was more accurate on a phylogram (Fig.~\ref{fig:results-distance}A, $r^2=0.95$) than on a chronogram (Fig.~\ref{fig:results-distance}B, $r^2=0.85$; Fisher's $z$-test, p=9.018e-11).
In contrast, for a trait evolving under a moving optimum, ancestral trait reconstruction was less accurate on a phylogram (Fig.~\ref{fig:results-distance}C, $r^2=0.79$) than on a chronogram (Fig.~\ref{fig:results-distance}D, $r^2=0.96$; Fisher's $z$-test, p=0).
...
Besides testing the fit of BM with a single-rate, we also fitted a multi-rate BM (see Methods).
...
This saturation effect is explored in supplementary material section~\ref{sec:saturation-analysis}, where we show that deeper phylogenies with higher nucleotide divergence exhibit stronger saturation of phenotypic divergence.
...
Gene expression levels (both mRNA and protein levels) represent one class of traits where distinguishing neutral evolution from selection is particularly important.
While the polygenic nature of expression traits can vary, testing whether their evolution is better explained by phylograms or chronograms can help determine whether changes are driven by neutral mutation accumulation or by selection responding to environmental changes.
...
Instead, we argue that phenotypic distance should be shown as a function of the square root of branch length in units of substitutions, not time.
...
We propose the phylogram-chronogram comparison as a new tool in the comparative methods toolbox, particularly useful when seeking deviation from neutral evolution.
In such cases, comparing the support for a phylogram versus a chronogram provides an additional diagnostic to assess whether the trait is genuinely evolving neutrally.
We provide a Bayesian model implemented in \textit{RevBayes} to perform this comparison, which estimates whether BM supports better a phylogram or a chronogram ($0 \leq \PhyloSupport \leq 1$), given the data at the tip of the tree and both trees with the same topology.
For empiricists more familiar with frequentist approaches, we also provide a maximum likelihood implementation that yields comparable results through AIC weights (see supplementary material section~\ref{sec:maximum-likelihood-estimation}).
...
Third, the proportionality constant in eq.~\ref{eq:covar-div} depends on the ratio of phenotypic to nucleotide mutation rates ($\MutationRatePheno / \MutationRateNuc$).
...
\begin{center}
    ...
    \includegraphics[width=0.8\textwidth, page=1] {figures/artwork_genet_architecture}
...
Importantly, the heritability $\Heritability$ is held constant across the phylogeny, meaning that environmental variance ($\VarEnv$) is adjusted proportionally to additive genetic variance ($\VarGenetic$) at each generation.
While this may seem counterintuitive, it represents a simplifying assumption that allows us to focus on the core question of phylogram versus chronogram support without introducing additional complexity from fluctuating heritability.
In reality, $\Heritability$ does fluctuate, but allowing it to vary freely would introduce additional parameters and complicate the interpretation of our results.
...
\newpage
\section{Saturation analysis}\label{sec:saturation-analysis}

In our simulation setting, the genetic architecture underlying the phenotype is constant along the phylogeny.
As a result, the phenotype may be bounded by what such genetic architecture can produce, and this could cause a slowdown of phenotypic divergence over time.
This phenomenon is analogous to nucleotide saturation, where deep phylogenies show reduced signal due to multiple substitutions at the same site~\citep{latrille_detecting_2024}.
...
This saturation effect can result in a spurious signal of OU for deeper phylogenies when the trait is evolving neutrally.

To explore this saturation effect, we performed additional simulations with varying levels of nucleotide divergence: 100 simulations with 4 times the divergence of the standard mammalian setting (high divergence); and
100 simulations with 20 times the divergence of the standard mammalian setting (very high divergence).

\begin{center}
    \begin{minipage}{0.49\linewidth}
        \flushleft {\tiny  \textbf{A}}
        \includegraphics[width=\linewidth, page=1]{figures/simulation_varGT_fastU_OU}
    \end{minipage}
    \begin{minipage}{0.49\linewidth}
        \flushleft {\tiny  \textbf{B}}
        \includegraphics[width=\linewidth, page=1]{figures/simulation_varGT_veryFastU_OU}
    \end{minipage}
    \captionof{figure}{
        \textbf{Support for OU under neutral trait evolution with different levels of saturation.}
        Violin plot of posterior parameter estimates for different models of trait evolution and different simulated regimes of selection.
        Horizontal lines inside the violin show the results of each replicate simulation.
        Simulations of 100 replicates per regime: trait evolving under a neutral regime (yellow) and under a moving optimum (blue).
        Wilcoxon rank-test are performed between the paired estimates on the phylogram and the chronogram.
        Panel~A \& B: Relative fit of an Ornstein-Uhlenbeck (OU) process compared to BM, using either a phylogram or a chronogram.
        Posterior estimates for the support of the OU process over the BM (reversible-jump).
        Panel~A: High divergence (4 times the standard mammalian setting).
        Panel~B: Very high divergence (20 times the standard mammalian setting).
        \label{fig:saturation-OU}
    }
\end{center}
