...
def plot_ancestral_traits(input_inference_tree: str, input_simu_tree: str, ax):
    ...
    for simu_node in simu_tree.traverse():
        if simu_node.name not in dict_inference.keys():
            # find the same node in the inference tree by matching the leaves names of the subtree
            leaves_names = set(simu_node.get_leaf_names())
            for node_inference in inference_tree.traverse():
                if set(node_inference.get_leaf_names()) == leaves_names:
                    dict_inference[simu_node.name] = node_inference
                    node_inference.name = simu_node.name
                    break
    ...
    if np.all(np.array(yerr) > 0):
        ax.errorbar(simu_nodes, inference_nodes, yerr=yerr, fmt='o', color='black', alpha=0.5)
    ...
    ax.plot([min(simu_nodes), max(simu_nodes)], [min(simu_nodes), max(simu_nodes)], color="black", linestyle="--",
            label="y=x")
    ax.set_xlabel("Phenotype_mean (simu)", fontsize=fontsize)
    ax.set_ylabel("Phenotype_mean (inference)", fontsize=fontsize)
    ax.set_title(os.path.basename(input_simu_tree.replace(".nhx.gz", "")), fontsize=fontsize)
...
def main(input_inference_tree_list: str, input_simu_tree_list: str, output_path: str):
    ...
    fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(ncols * 1920 / my_dpi, nrows * 1920 / my_dpi),
                             dpi=my_dpi)
    i = 1
    r2_list = []
    for input_inference_tree, input_simu_tree in zip(input_inference_tree_list, input_simu_tree_list):
        ...
        seed_inference = int(os.path.basename(os.path.dirname(input_inference_tree)).split("_")[-1].replace("seed", ""))
    ...
    ax.set_title("Histogram of R² values", fontsize=fontsize)
    ax.set_xlim(0, 1)
