...
class MutationRateProcess {
  ...
  public:
    ...
    void update(double scaling = 1.0) {
        if (brownian_sigma != 0.0) {
            brownian += scaling * brownian_sigma * normal_distrib(generator_mut_rate);
...
class Population {
  public:
    ...
    void run(u_long nbr_generations, Trace &trace, bool burn_in = false, double scaling = 1.0) {
        ...
        for (u_long sample{1}; sample <= nbr_generations; sample++) {
            ...
            if (not burn_in) {
                pop_size.update();
                mutation_rate.update(scaling);
            }
            trace.add("Lineage", name);
            trace.add("Generation", elapsed);
            trace.add("Scaling", scaling);
...
class Process {
  ...
  public:
    explicit Process(Tree &tree, Population const &pop, Trace &trace) : tree{tree} {
        ...
        run_recursive(tree.root(), trace, 1.0);
        timer_count();
    }


    // Recursively iterate through the subtree.
    void run_recursive(Tree::NodeIndex node, Trace &trace, double scaling) {
        // Substitutions of the DNA sequence is generated.

        if (!tree.is_root(node)) {
            double total_length = tree.total_length();
            populations.at(node).run(static_cast<u_long>(tree.node_length(node)), trace, false, scaling);
            generations_computed += tree.node_length(node);
            cout << generations_computed << " generations computed in total ("
                 << static_cast<int>(100 * generations_computed / total_length)
        ...
        for (auto &child : tree.children(node)) {
            ...
            run_recursive(child, trace, scaling);
        }
    }
